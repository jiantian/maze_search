...
def solver(filename, option):
	...
	if option == 'A':
		print 'A star search:'
		Astar(maze, start, end, nodes, frontier, path, visited)
...
def Astar(maze, start, end, nodes, frontier, path, visited):
    found = False
    parents = {}
    parents[tuple(start)] = None
    pq = []
    ...
    while (len(pq) > 0 and found == False):
        ...
        if (len(neighbors) > 0):
            for neighbor in neighbors:
                parents[tuple(neighbor)] = node
                if (AstarHeuristics(neighbor, end, parents), neighbor) not in pq:
                    heappush(pq, (AstarHeuristics(neighbor, end, parents), neighbor))
...
def AstarHeuristics(node, end, parents):
	return len(getPath(node, parents))-1 + manhattan(node, end) 
